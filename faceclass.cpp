...
void faceclass::pedestriandect(cv::Mat& image, bool usegaussforegroundfordect)
{
...
//默认创建LBP人脸识别模型，可以通过参数修改
{
	...
	case 2:
		...
		break;
	case 3:
		_model = createLBPHFaceRecognizer();//设置为LBP模型，此模型检测人脸效果最好
...
void faceclass::predect(bool usegaussforegroundfordect, bool dect_face, bool use_camshift, bool dect_pedestrian, bool save_videobool)
{
	...
	if (opencamera())//如果成功打开摄像头
	{
		...
		while (_capture.read(_frame)){
			...
			if (usegaussforegroundfordect){
				guassforeground(_frame, 0.05, false);
				int forenum = getforegroundrect();
				if (forenum){
					if (dect_pedestrian){//如果检测行人
						pedestriandect(_frame, usegaussforegroundfordect);//在当前帧中检测行人
					}
					if (dect_face){
						int facenum = facedect(_frame, usegaussforegroundfordect);//在当前帧中检测人脸
			...
			else if (!usegaussforegroundfordect && ispre){//如果不使用背景检测方法，且用户按下了字母p键，开始检测并识别人脸			
